...
export class ReportBehaviorComponent implements OnInit {
  ...
  public setView(newView: EnumPageView) {
    this.view = newView;
    if (this.view === EnumPageView.Search && this.autocompleteBehaviors.length === 0) {
      this.getAutocompleteBehaviors();
  ...
  public toggleBehaviorPicking(name: string, force?: boolean) {
    ...
    let found = this.behaviors.find(behavior => behavior.name === name);
    if (found) {
  ...
  public addBehavior(name: string) {
    this.apiService
      ...
      .subscribe(
        (data) => {
          let behavior = data;
          behavior.recommended = false;
          behavior.existing = true;
          behavior.picked = true;
  ...
  public submit() {
    let report = this.behaviors.filter(symptom => symptom.picked).map(symptom => symptom.id);
    if (!report.length) {
      return;
    }
  ...
  private getAutocompleteBehaviors(): void {
    this.apiService
      .request('get', `api/v2/behaviors?all=true`)
      .subscribe(
        (data: {customized_behaviors: any, official_behaviors: any}) => {
          this.autocompleteBehaviors = data.official_behaviors.concat(data.customized_behaviors);
        },
        (err: any) => {
          console.log(err);
          // TODO: do something
  ...
  private getBehaviorMetric(): void {
    this.apiService
      .request('get', 'api/metrics/behavior')
      .subscribe(
        (data) => {
